...
esp_err_t bt_source_init(void) {
    ...
    // Initialize AVRCP Controller FIRST (required before A2DP)
    ret = esp_avrc_ct_register_callback(bt_avrc_ct_cb);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "AVRC callback register failed: %s", esp_err_to_name(ret));
    }
